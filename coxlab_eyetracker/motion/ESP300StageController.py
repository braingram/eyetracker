...
class ESP300StageController(IPSerialBridge):
    ...
    def move_composite_absolute(self, axes, new_positions):
        # should produce a simultaneous movement on multiple axes,
        ...
        if(len(axes) == 2):
            ...
            # set the group velocity, acceleration, and decelleration to
    ...
    def move_composite_relative(self, axes, new_positions):
        ...
        for i in range(0, len(axes)):
            ...
            target_positions.append(float(self.current_position(axes[i])) + \
                    float(new_positions[i]))
    ...
    def current_position(self, axis):
        ...
        while (not okayp and retries > 0):
            ...
            #print "result_string in current_position in while loop:", \
            #        result_string
        ...
        #log.error("current_position(%s): %s" % (axis, result_string))
        try:
            if len(result_string) > 1:
                #print "first element of result_string in current_position:", \
                #        result_string
            ...
            log.error("current_position error: %s" % E)
            return numpy.inf
