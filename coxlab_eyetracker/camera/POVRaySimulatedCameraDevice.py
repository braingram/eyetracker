...
class POVRaySimulatedCameraDevice:
    def __init__(self, _feature_finder, _stages, _leds, _d, **kwargs):
        ...
        # the stage has no concept of distance to the eye
        # so this must be spec'd
        self.d = None
    ...
    def acquire_image(self):
        ...
        cam_look = cam_pos + array([sin(radians(r)), \
                sin(radians(self.droop_angle)), cos(radians(r))])
        ...
        #cam_right = array([cos(radians(self.tilt)), \
        #        sin(radians(self.tilt)), 0.0])
...
#        if(self.quiet):
#            command_string = "/sw/bin//povray -D +A +O/tmp/eyeball.png +W%d "
#            "+H%d /tmp/eyeball.pov  >& /dev/null" % (self.w, self.h);
#        else:
#            command_string = "/sw/bin//povray +O/tmp/eyeball.png +A +W%d "
#            "+H%d /tmp/eyeball.pov" % (self.w, self.h);
#            print "Command = " + command_string

        if(self.quiet):
            command_string = "/usr/local/bin/povray -D +A " \
            "+O/tmp/eyeball.png +W%d +H%d /tmp/eyeball.pov  >& /dev/null" % \
            (self.w, self.h)
        else:
        ...
        if self.add_noise:
            ...
            self.im_array += self.noise_level * \
                    random.randn(im_shape[0], im_shape[1])
        ...
        self.feature_finder.analyze_image(self.im_array.copy(), \
