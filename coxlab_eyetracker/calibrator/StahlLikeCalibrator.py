...
class StahlLikeCalibrator:
    ...
    def __init__(self, camera, stages, focus_and_zoom, leds, **kwargs):
        ...
        #self.default_Rp = kwargs.get("default_Rp", 6. * \
        #        self.default_pixels_per_mm)
    ...
    def save_parameters(self, filename, controller):
        ...
        if self.top_led in self.default_cr_positions.keys():
            calibrator_info['default_cr_top'] = \
                    self.default_cr_positions[self.top_led]

        if self.side_led in self.default_cr_positions.keys():
            calibrator_info['default_cr_side'] = \
                    self.default_cr_positions[self.side_led]
    ...
    def set_parameters(self, parameters, controller):
        ...
        #  "self.Rp != None and self.y_equator != None \
        #             and self.y_topCR_ref != None and \
        #             self.side_led in self.default_cr_positions.keys() and \
        #             self.top_led in self.default_cr_positions.keys())
        # print self.d
    ...
    def acquire_averaged_features(self, n, retry=200):
        ...
        # Flush queue (somehow a buffer of images is stored during
        # acquisition and must be flushed)
        for i in range(0, 3):
        ...
        pupil_radius = array([])
        cr_radius = array([])
        # this first row is just for initialization
        # (it should be removed once the array is filled)
        pupil_position = zeros((1, 2))
        ...
        while n_count < n and n_attempt < retry:
            self.camera.acquire_image()
            features = self.camera.get_processed_image()
            # Testing on the key 'pupil_position' is enough to
            # guarantee that all other relevant parameters exist
            if features is not None and features['pupil_position'] is not None:
                ...
                pupil_position = vstack((pupil_position, \
                        features['pupil_position']))
        ...
        if pupil_radius.shape[0] > 0:
            ...
            #cr_position = \
            #        self.CompLensDistor.fully_compensate(cr_position, None)
            #cr_position = cr_position.transpose()
            #pupil_position = pupil_position.transpose()
            #pupil_position = \
            #        self.CompLensDistor.fully_compensate(pupil_position, None)
            #pupil_position = pupil_position.transpose()
    ...
    def calibrate(self):
        """Calibrate the eye tracker manipulating stages and leds as needed.
            ...
            Following this call, the transform method will convert image
    ...
    def transform(self, pupil_coordinates, cr_coordinates):
        ...
        if self.leds.soft_status(self.side_led) and \
                self.leds.soft_status(self.top_led):
        ...
        if calibration_status is self.uncalibrated:
            return pupil_coordinates[0], pupil_coordinates[1], \
                    calibration_status
        ...
        if which_led == self.side_led:
            cr_coordinates = cr_coordinates - \
                    self.default_cr_positions[self.side_led] + \
                    self.default_cr_positions[self.top_led]

        y_equator = self.y_equator + (cr_coordinates[:, self.y_image_axis] - \
                self.y_topCR_ref)
        ...
        azimuth = arcsin((pupil_coordinates[:, self.x_image_axis] - \
                cr_coordinates[:, self.x_image_axis]) / \
                sqrt(Rp ** 2 - y_displacement ** 2)) * 180 / pi
    ...
    def center_horizontal(self):
        ...
        if self.top_led in self.default_cr_positions:
            self.y_topCR_ref = \
                    self.default_cr_positions[self.top_led][self.y_image_axis]

    def center_vertical(self):
        """Vertically align the camera with the eye"""
        self.center_axis(self.y_stage_axis)

        # Save the position of the CR spot with the light on the side:
        # this is the y coordinate of the equator
        if self.side_led in self.default_cr_positions:
            self.y_equator = \
                    self.default_cr_positions[self.side_led][self.y_image_axis]
    ...
    def _jogged_cr_difference(self, d, rs, axis):
        ...
        cr_pos = []
        for i in range(0, 2):
            d_new, reversal_function = \
                    self.stages.composite_rotation_relative(d, rs[i])

            features = self.acquire_averaged_features( \
    ...
    def _jogged_pupil_cr_difference(self, d, r, axis):
        ...
        d_new, reversal_function = \
                self.stages.composite_rotation_relative(d, r)
        features = self.acquire_averaged_features(self.n_calibration_samples)
    ...
    def center_depth_faster(self):
        """"
        Fit a linear function to the depth vs. cr-displacement
        ...
        # 2. Sample some distances in the range
        # self.d_guess +/- self.d_half_range
        n_points_to_sample = 4
        ds = linspace(self.d_guess - self.d_halfrange, \
                self.d_guess + self.d_halfrange, n_points_to_sample)
        ...
        for d in ds:
            motion_func, d_new = \
                    self.stages.precompute_composite_rotation_relative( \
                    d, self.jog_angle)
        ...
        for precomputed_motion in precomputed_motions_pos:
            precomputed_motion()
            features = \
                    self.acquire_averaged_features(self.n_calibration_samples)
            cr_pos = features["cr_position"]
        ...
        measured_cr_displacements = array(measured_cr_displacements_pos) - \
                array(measured_cr_displacements_neg)
    ...
    def center_depth(self):
        """"
        Vary the radius of rotation of the camera, untill the CR spot is
        stable in the image (i.e., it does not move when the camera rotates
        to the left or to the right).

             This is achieved with an adaptive search loop that stops when
             the CR difference is small enough or a maximum # of loops
             are executed.
        """
        ...
        range = [self.d_guess - self.d_halfrange, \
                self.d_guess + self.d_halfrange]

        objective_function = \
                lambda x: abs(self._jogged_cr_difference( \
                x, rs, self.x_image_axis))
        self.d = scipy.optimize.fminbound( \
                objective_function, range[0], range[1], (), \
    ...
    def autofocus(self):
        ...
        focus_target = scipy.optimize.fminbound( \
                self.sharpness_objective, focus_range[0], \
                focus_range[1], (), 0.1, 20)
    ...
    def center_depth_old(self):
        ...
        # 2. Adaptive loop
        #   a. Rotate the camera left/right and compute the
        #       difference in the x location of the CR
        #   b. Compute the new value of the rotation radius
        #       (d) - closer to the center of the eye
        #   c. Move the stages to correct

        d = self.z_delta
        # initialize the distance to the approximate (from config_dict
        d = self.d_guess
        rs = [-self.jog_angle, self.jog_angle]

        # jog the camera back and forth r degrees
        # rotating about a point dist away
    ...
    def align_pupil_and_CR(self):
        """
        Vary the angle of rotation of the camera,
        untill the CR and the pupil are horizontally aligned.

            This is achieved with an adaptive search loop that stops when the
            difference between the x coordinate of the CR and pupil is
            smallenough or a maximum # of loops are executed.

            Side effects: camera will be moved so that the CR and pupil are
        ...
        # 2. Adaptively optimize (this actually moves the stages)
        range = [-20., 20.]
        objective_function = lambda x: abs(self._jogged_pupil_cr_difference( \
                self.d, x, self.x_image_axis))
        r = scipy.optimize.fminbound(objective_function, range[0], range[1], \
                (), self.cr_diff_threshold, 30)

        # 3. Move the camera in the final position with pupil and CR aligned
        self.d, reversal_function = \
                self.stages.composite_rotation_relative(self.d, r)
    ...
    def align_pupil_and_CR_manual(self, r):
        ...
        self.d, reversal_function = self.stages.composite_rotation_relative( \
                self.d, r)
        features = self.acquire_averaged_features(self.n_calibration_samples)
        cr_pos = features["cr_position"]
        pupil_pos = features["pupil_position"]
    ...
    def find_pupil_radius(self):
        """
            Compute Rp, the distance from the center of the corneal
            curvature to the pupil

            This is accomplished by rotating the camera around the eye and
            measuring the image displacement
            of the stationary pupil

            Side effects: self.Rp is set to the computer Radius of the pupil's
        ...
        #    point in the calibration we are able to rotate the camera about
        ...
        # precompute the movements so that we can make them faster and in
        # succession
        precomputed_motions = []
        true_distances = []
        for r in rs:
        ...
        for i in range(0, len(precomputed_motions)):
            ...
            pup_radiuses.append(features["pupil_radius"] / \
                    relative_magnification)
        ...
        y_displacement = pupil_pos[self.y_image_axis] - \
                cr_pos[self.y_image_axis]
        ...
        self.Rp = self._compute_Rp(x_displacements, radians(rs), \
                y_displacement)
        self.Rp_mm = self.Rp / self.pixels_per_mm
        ...
        self.leds.turn_on(self.top_led)
        self.leds.turn_off(self.side_led)
        # Save the final y value of the CR as a reference to measure
        # how much y displacement we get during eye tracking
        features = self.acquire_averaged_features(self.n_calibration_samples)
        cr_pos = features["cr_position"]
    ...
    def _compute_Rp(self, x_displacements, angle_displacements, \
        ...
        # eye's "equator"
        p0 = (6.0, radians(0))
        leastsq_results = scipy.optimize.leastsq(self.residuals_sine, \
                p0, (x_displacements, angle_displacements))
        p = leastsq_results[0]
        Rp_prime = p[0]
        angle_offset = p[1]
    ...
    def find_center_camera_frame(self):
        ...
        m_top = (cr_pos_top_zoom[0] - cr_pos_top[0]) / \
                (cr_pos_top_zoom[1] - cr_pos_top[1])
        p_top = cr_pos_top[0] - m_top * cr_pos_top[1]
        m_side = (cr_pos_side_zoom[0] - cr_pos_side[0]) / \
                (cr_pos_side_zoom[1] - cr_pos_side[1])
    ...
    def fit_pupil_radius_size(self):
        """
            Repeat the computation of Rp for different intensities of the
            visible light (i.e., puil sizes)

            A linear fit between measured Rps and pupil sizes is performed to
            obtain the relationship between these variables.
        """
