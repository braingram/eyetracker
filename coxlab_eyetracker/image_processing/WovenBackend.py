...
class WovenBackend(VanillaBackend):
            ...
            for(int r = 0; r < h; r++){
                for(int c = 0; c < w; c++){
                    ...
                    for(int k = 0; k < row_width; k++){
                        ...
                        if(k_index < 0) k_index = -(k_index + 1);  // reflect at boundaries
                        ...
                        __TYPE kernel_coef = *((__TYPE*)((char *)row_array->data + row_array->strides[0] * (row_width - k - 1)));
            ...
            for(int c = 0; c < w; c++){
                for(int r = 0; r < h; r++){
                    ...
                    for(int k = 0; k < col_width; k++){
                        ...
                        __TYPE kernel_coef = *((__TYPE *)((char *)col_array->data + col_array->strides[0]*(col_width - k - 1)));
                        *result_ptr += kernel_coef * (*image_ptr);
    ...
    def fast_radial_transform(self, image, radii, alpha, **kwargs):
        ...
        for r in range(0, len(radii)):
            ...
            # Generate a Gaussian of size proportional to n to
            # smooth and spread
